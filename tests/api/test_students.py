...
async def test_student_complete_crud_lifecycle(client: AsyncClient):
    ...
    assert create_response.status_code == 201, (
        f"Failed to create student: {create_response.text}"
    )
    ...
    assert student_data["grade"] == "A"

    assert student_data["details"]["address_line_1"] == "123 MG Road"
    assert student_data["details"]["address_line_2"] == "Near Railway Station"
    ...
    fetch_response = await client.get(
        f"/api/v1/fetch-one-student/{student_id}"
    )
    ...
    assert fetched_student["phone_number"] == "9876543210"
    assert fetched_student["roll_number"] == "CS-101"
    assert fetched_student["grade"] == "A"
    ...
    assert updated_student["roll_number"] == "CS-101"
    ...
    delete_response = await client.delete(
        f"/api/v1/delete-student/{student_id}"
    )
